...
\begin{abstract}
Computational phenotyping of agency has two halves: fitting a generative model to
behaviour, and reading agency-relevant quantities off the fitted model. Prior work
established the second half on hand-specified models and left the first as a
companion sketch. We complete the loop. A matrix product state (an interpretable
tensor network) is trained unsupervised on action--observation rollouts of a T-maze
partially observable Markov decision process, so its generative model $p(o,a)$ is
learned from data rather than stipulated; we then recover the model's structure in
five senses that a bare density estimate does not provide. From the learned network
...
Agency in artificial systems is increasingly asserted and rarely measured. A
companion line of work~\cite{wilsonAgency2025} argues that a minimal, operational
...
to behavioural data, and (ii) explain variation from the inferred parameters. The
worked example performed only the second half, on models specified by hand. This
paper performs the first half, and shows that once it is done the second half returns
not just a number but the entire interpretable model behind it.
