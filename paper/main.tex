...
Empowerment computed on the recovered model tracks the three control regimes of the
first paper to three decimals, and rises when the agent acts to resolve its uncertainty
even though nothing in training rewards that. Since the tensor network is not itself an
...
Agency in artificial systems is asserted far more often than it is measured. Our first
paper~\cite{wilsonAgency2025} (Paper~I) held that a useful notion of agency turns on
three properties, intentionality, rationality, and explainability, and argued that
active inference~\cite{parr2022} supplies all three at once: within a partially
observable Markov decision process (POMDP) the agent's posteriors, its prior
preferences, and its expected-free-energy-driven policy give beliefs, desires, and a
normative reason for acting, each inspectable in the likelihood and transition tensors.
On such a model, empowerment~\cite{klyubin2005,salge2014}, the channel capacity from an
action to the next observation, measures how much of what the agent will observe its
actions can actually decide. This paper is the follow-up. It keeps the definition, the
probe, and the T-maze of Paper~I fixed, and adds the step Paper~I set aside.
...
fitted parameters. Paper~I did only the second, on models we specified by hand, and
described the first as work to come. We do it here. Once the model is learned from
behaviour, the reading that produces an empowerment number also produces the entire
generative model that stands behind it.
...
We borrow one quantity from Paper~I, $\max_{p(a)} I(A;O')$, the largest mutual
information an agent can drive between its next action and its next observation,
evaluated by Blahut--Arimoto~\cite{salge2014}. Paper~I distinguishes the value computed
under the true environment (\emph{objective}) from the value under the agent's beliefs
(\emph{subjective}), and both from the \emph{actual} value once preferences narrow the
action distribution. Only one variant is empirical here: computed under the learned
