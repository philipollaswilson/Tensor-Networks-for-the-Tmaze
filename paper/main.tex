...
\title{Structure Learning of Generative Models for Agency Phenotyping
with Tensor Networks}
\titlerunning{Structure Learning for Agency Phenotyping}
...
\begin{abstract}
An agentic system, we argued in earlier work~\cite{wilsonAgency2025}, should be
intentional, rational, and explainable; active inference secures the three in a
partially observable Markov decision process whose tensors are open to inspection, and
empowerment then reads off how much control the agent has over what it will observe.
That case was built on models we wrote by hand. Its open question was whether the model,
and the agency-relevant structure inside it, could instead be recovered from behaviour.
This paper answers it, and the answer turns out to be an entire interpretable model, not
a single number. A matrix product state (an interpretable tensor network) is trained
unsupervised on T-maze rollouts, and from the trained network we recover, each validated
against the true environment: normalised hidden states, by a gauge-invariant criterion
of predictive equivalence that earlier tensor-network models could not supply; the
labelled likelihood $A=p(o\mid s)$ and transition $B=p(s'\mid s,a)$, matched to
total-variation $0.000$; the conditional-independence graph among observables, by exact
mutual information; the factorisation of the observation, in which context is recovered
as a near-independent modality; and the number of hidden states, fixed by model
selection read as active-inference model expansion and Bayesian model reduction. A gauge
transformation then makes the bond index itself a labelled state. Because the recovered
model is fully interpretable, the controllability phenotype now reads off a learned
model rather than a stipulated one. The construction assumes no active-inference agent,
and so transfers to any system whose sensorimotor stream can be modelled.
...
Agency in artificial systems is asserted far more often than it is measured. We argued
in earlier work~\cite{wilsonAgency2025} that agency worth the name is intentional,
rational, and explainable, that active inference~\cite{parr2022} secures the three at
once in a POMDP whose likelihood and transition tensors are open to inspection, and that
empowerment~\cite{klyubin2005,salge2014}, the channel capacity from an action to its
next observation, measures the control an agent holds over what it will see. That
argument ran on generative models we specified by hand. What it left undone was to
obtain such a model, and the agency-relevant structure inside it, from behaviour.

That is the problem this paper solves, and it is a problem in \emph{structure learning}
of a generative model, not in learning ``agency'' directly. Agency sets the target: the
structure worth recovering is exactly what the three criteria and the empowerment probe
read off a world model, so that once the model is learned the phenotype runs on it. Phenotyping,
taken from computational psychiatry~\cite{montague2012,schwartenbeck2016}, has two
steps: fit a generative model to behaviour, then read behaviour off the fitted
parameters. Earlier we did only the second, on models specified by
hand~\cite{wilsonAgency2025}; here we do the first, and find that the fitted model can
be made fully interpretable rather than left as a black-box density.
...
probe, and the design of the T-maze are developed in full in our earlier
paper~\cite{wilsonAgency2025}, and the tensor-network machinery is new here.
...
\caption{The T-maze, reproduced from~\cite{wilsonAgency2025}. The arrow marks the
observation revealing where the cheese is; entering an arm is an absorbing trap, so the
epistemically optimal first move is the cue.}
\label{fig:tmaze}
\end{figure}

\subsection{What is learned, and from what}
The training set is the space of action--observation trajectories the environment can
produce, not the log of any one agent. We enumerate every action sequence and pair it
with the observations the environment returns, weighting each trajectory by its true
probability, so the action distribution is uniform and carries no policy. What the MPS
fits is therefore the environment's own input--output law, and the structure we later
recover is the environment's generative model (its hidden states, likelihood, and
transition), which is exactly why it can be checked against the analytic $q(o,a)$.
Learning instead from a \emph{particular} agent's rollouts would recover the model that
agent's policy induces, a different and agent-specific object; we return to this in
Sec.~\ref{sec:discussion}. Empowerment is indifferent to the choice, as it already
maximises over the action distribution.
...
We borrow one quantity from that work~\cite{wilsonAgency2025}, $\max_{p(a)} I(A;O')$,
the largest mutual information an agent can drive between its next action and its next
observation, evaluated by Blahut--Arimoto~\cite{salge2014}. It distinguishes the value
computed under the true environment (\emph{objective}) from the value under the agent's
beliefs (\emph{subjective}), and both from the \emph{actual} value once preferences
narrow the action distribution. Only one variant is empirical here: computed under the learned
model, the subjective value approaches the objective one as the fit tightens. We report
it over the full observation and over the reward modality alone.
...
intrinsic labelling, so any state we read off it must be invariant to that freedom.
Each history $h$ (a prefix of actions and observations) drives the network into a bond
vector $|b_h\rangle$; the quantity $|\langle b_i|b_j\rangle|^2$ is gauge-invariant, and
grouping histories by it gives a first partition into states
...
$p(o'\mid a',h)$. Agglomerating the action-conditioned predictive signatures and merging
any two whose futures agree to within an L1 tolerance (Fig.~\ref{fig:states}, right)
collapses the gauge duplicates: the four context-bit pairs merge at $\approx 0$
distance, the two start histories at $1.2$, and the seven genuine states stay $\ge 2.0$
apart, so any threshold in that gap returns them (unresolved start, four arm--outcome
traps, two cue contexts). The count does not hang on the threshold: it is corroborated
by the bond-dimension model selection of Sec.~\ref{sec:modelsel} and by the fact that
all seven recovered emissions match ground truth (Sec.~\ref{sec:ab}). What the learned
latent encodes is therefore distinguishability rather than raw history: whether the agent
entered from the left or the right arm collapses into one state when position cannot be
seen, and splits into two once it can.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig_states.png}
\caption{Recovering states beyond bond-ray fidelity. Left: on the full maze, the
fidelity clustering used in our earlier pipeline~\cite{wilsonAgency2025} over-separates
into nine apparent blocks (twelve histories), because the model writes the uninformative
context bit into the bond as a coherence that does not affect the future. Right:
predictive equivalence merges histories whose predicted futures agree; the gauge
duplicates collapse at $\approx 0$ distance while the seven genuine states stay
$\gtrsim 2$ apart, so a threshold in the gap recovers the correct count.}
\label{fig:states}
\end{figure}
...
The reconstruction matches ground truth to measurement precision
(Fig.~\ref{fig:ab}, Table~\ref{tab:results}).
...
\paragraph{Only the cue is informative.} Labelling the arm states by outcome
(\texttt{R/cheese} versus \texttt{R/shock}) raises a fair question: does the model treat
seeing Cheese or Shock at an arm as resolving the hidden context, which would make a
blind arm entry an epistemic move rather than a gamble? It does not, and the recovered
emission shows why. The full $p(o_2\mid s_2)$, factored into Position, Reward, and
Context, leaves the context factor at $0.50/0.50$ for every arm state
(Fig.~\ref{fig:emission}): the two outcomes are distinct predictive states because they
emit distinct \emph{rewards}, but neither carries information about \emph{context}. Made
quantitative, the mutual information between a blind arm's outcome and the start context
is $0.000$ bits, so the context posterior after entering Right blind is unchanged at
$0.50/0.50$; the same measure at the cue is $1.000$ bit, where the observed context bit
determines the reward of the next arm. The learned model reproduces both to within
$10^{-3}$. This is the intended information geometry of the task---only the cue reduces
uncertainty about the reward contingency---recovered from behaviour rather than assumed.

\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{figs/fig_emission.png}
\caption{Emission $p(o_2\mid s_2)$ of the learned full-maze model, factored into
Position, Reward, and Context. The four arm states (boxed) are separated by their reward
but leave context at $0.50/0.50$: a blind arm carries $I(\text{reward};\text{context})=0$
bits, so it resolves no hidden state. Only the two cue states fix the context, where the
read-off bit is worth $1$ bit about the next arm's reward.}
\label{fig:emission}
\end{figure}
...
from the converged full-maze network reproduces the analytic regimes to three decimals
(Fig.~\ref{fig:empower}, left; Table~\ref{tab:results}): $2.00$ bits at the
start, $0.00$ in the trap, and $2.00$ bits (full) / $1.585$ bits (reward) post-cue,
against analytic $2.00$/$1.585$. Restricting the probe to the reward modality returns
the three control regimes of the minimal maze ($1.00$/$0.00/1.585$ bits): reading
empowerment only over outcomes the agent cares about isolates the control that matters
for behaviour, which the full observation space dilutes. The subjective--objective gap is now empirical and small:
a bond-truncated checkpoint attenuates post-cue empowerment ($0.65$ bits reward-only),
and it approaches the objective value as the fit improves. Acting to resolve
uncertainty increases empowerment: visiting the cue lifts it from the intermediate to
the maximal regime, though empowerment is no part of the training objective.
...
construction, gauge-invariant and based on predictive equivalence, and build on it a
complete structural account: a labelled likelihood and transition validated to
$\sim\!10^{-3}$, the conditional-independence graph, the observation factorisation, a
principled state count, and a symbolic-state gauge. The controllability probe then runs
on the learned model and matches the analytic regimes. The pipeline is architecture-agnostic:
...
\paragraph{What the agency claim is, and is not.} The contribution to agency is the
recovered model, not the empowerment number. The states, the likelihood, and the
transition are the belief-like and world-model structures that the intentionality,
rationality, and explainability criteria are read off; recovering them from behaviour is
what lets those criteria reach a system whose internals were opaque. Empowerment adds
only the controllability facet, whether the recovered interface leaves the agent's
economy anything to decide, and on its own it is not a measure of agency
(Sec.~\ref{sec:empower}). Two caveats bound even this. Because the model is fit to
uniform-action rollouts, what we recover is the world's structure, the ideal belief an
accurate agent would hold, not any particular agent's posterior; and we exercise the
controllability probe rather than the full three-criteria assessment, which the recovered
model enables but which we leave to a study with agent-specific data.
...
memorise, would strengthen the claim. The model here is fit to exhaustive, uniform-action
rollouts, so it recovers the environment's generative structure; phenotyping a
\emph{particular} agent means fitting to that agent's own rollouts, where the recovered
model reflects its policy and where coverage of the state space is no longer guaranteed.
Empowerment measures controllability, not agency:
...
arXiv:0811.4413
\bibitem{srinivasan2021} Adhikary, S., Srinivasan, S., Miller, J., Rabusseau, G.,
Boots, B.: Quantum Tensor Networks, Stochastic Processes, and Weighted Automata. In:
AISTATS. PMLR, vol. 130, pp. 2080--2088 (2021). arXiv:2010.10653
