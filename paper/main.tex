...
\begin{abstract}
This paper is the sequel to our study of agency in AI systems, which set out the
properties an agentic system should have and used empowerment to gauge how much control
an agent holds over its own future. That study measured models we built by hand;
fitting a model to behaviour, the other half of computational phenotyping, was left as
a plan. We carry it out. A matrix product state, an interpretable tensor network, is
trained unsupervised on action--observation rollouts of a T-maze, so its generative
model $p(o,a)$ is learned from behaviour rather than written down by us. We then read
off the trained network several kinds of structure that a plain density estimate leaves
hidden. Normalised hidden states come first, recovered by a gauge-invariant criterion
of predictive equivalence, where earlier tensor-network work found the bonds ``not
normalised and therefore not usable'' and stopped. From the states we rebuild the
labelled likelihood $A=p(o\mid s)$ and transition $B=p(s'\mid s,a)$, which match the
environment to total-variation $0.000$ and reproduce its branching exactly; the
conditional-independence graph, from exact mutual information; and the observation's
factorisation, in which context appears as a near-independent modality. The number of
hidden states is fixed by a model-selection argument, read as active-inference model
expansion and Bayesian model reduction rather than off a truncation threshold.
Empowerment computed on the recovered model tracks the three control regimes of that
earlier study to three decimals, and rises when the agent acts to resolve its
uncertainty even though nothing in training rewards that. Since the tensor network is not itself an
active-inference agent, the method carries over to any system whose sensorimotor stream
can be modelled.
...
in earlier work~\cite{wilsonAgency2025} that a useful notion of agency turns on three
properties, intentionality, rationality, and explainability, and that active
inference~\cite{parr2022} supplies all three at once: within a partially observable
Markov decision process (POMDP) the agent's posteriors, its prior preferences, and its
expected-free-energy-driven policy give beliefs, desires, and a normative reason for
acting, each inspectable in the likelihood and transition tensors. On such a model,
empowerment~\cite{klyubin2005,salge2014}, the channel capacity from an action to the
next observation, measures how much of what the agent will observe its actions can
actually decide. This paper is the follow-up: it keeps that definition, the probe, and
the T-maze fixed, and adds the step we set aside.

Phenotyping, taken from computational psychiatry~\cite{montague2012,schwartenbeck2016},
runs in two steps: fit a generative model to behaviour, then read behaviour off the
fitted parameters. We did only the second there, on models specified by hand, and left
the first as work to come. We do it here. Once the model is learned from behaviour, the
reading that produces an empowerment number also produces the entire generative model
...
traps, two cue contexts). What the learned latent encodes is therefore
distinguishability rather than raw history: whether the agent came from the left or the
right arm collapses into one state when position cannot be seen, and splits into two
once it can.
